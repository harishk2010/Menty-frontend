...
//         </p>
//         <p className="text-sm text-gray-500">
//           Logged in as: <span className='text-sm text-purple-500 font-semibold'>{currentUser.name}</span>  ({currentUser.role})
//         </p>
//       </div>
...
import { Send, Image, X } from 'lucide-react';
import { getChatHistoryById, uploadChatImage } from '@/api/chatApi';
import { getStudentData, getStudentDataById } from '@/api/studentApi';
...
const ChatInterface = ({ 
  ...
  const [recipientRole, setRecipientRole] = useState<'student' | 'instructor'>('student');
  ...
  const fetchRecipientDetails = async () => {
    try {
      ...
      if (currentUser.role === 'student') {
        // If current user is a student, fetch instructor details
        recipientDetails = await getInstructorDataById(instructorId);
        setRecipientName(recipientDetails.username);
        setRecipientRole('instructor');
      } else {
        // If current user is an instructor, fetch student details
        recipientDetails = await getStudentDataById(studentId);
        setRecipientName(recipientDetails.username);
  ...
  // Fetch recipient details when component mounts or user changes
  useEffect(() => {
    if (currentUser) {
    ...
    <div className="flex flex-col h-[500px] bg-white rounded-lg shadow-lg">
      ...
      <div className="p-4 border-b">
        ...
        <div className="text-sm text-gray-500">
          Logged in as: 
          <span className='text-sm text-purple-500 font-semibold ml-1'>
            {currentUser.name} ({currentUser.role})
          </span>
          <span className="ml-3">
            Chatting with: 
            <span className='text-sm text-orange-500 font-semibold ml-1'>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {isTyping && (
          <div className="text-sm text-black max-w-[10%] rounded-lg p-2 text-wrap">
            typing...
          </div>
        )}
        <div ref={messagesEndRef} />
