...
const CourseDetails: React.FC = () => {
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const instructorData = await getInstructorDataById(courseResponse.mentorId);
        if (!instructorData) {
          toast.error("Instructor Data Not Found");
          return;
        }
        ...
        const purchaseData = await isBoughtCourse(userData?.userId, courseId);
        setPurchased(purchaseData.data.isBought);

        // Fetch chapters
        const courseChapters = await getAllChaptersDetails(courseId);
        setChapters(courseChapters.data || []);

        // Fetch reviews
        const reviewsResponse = await GetCourseReviews(courseId);
        setReviews(reviewsResponse.data.reviews || []);
        setRatingAverage(reviewsResponse.data.averageRating || 0);
  ...
  // If loading or data is not fully loaded, show loader
  if (loading || !course || !instructor) return <Loading />;
                ...
                {showControls && (
                  <>
                    <motion.div
                      ...
                      animate={{ y: 0, opacity: 100 }}
                      transition={{ ease: "easeInOut", duration: 0.3 }}
                      className="absolute bottom-10 left-1/2 transform -translate-x-1/2 flex space-x-2 justify-center items-center"
                    >
                      <button
                        onClick={restartVideo}
                        className="transparent hover:bg-purple-500 transition ease-in-out duration-400 rounded-full p-1"
              ...
              {activeSection === "reviews" && (
                <div>
                  ...
                  <div>
                    <button
                      onClick={handleReviewFormToggle}
                      className="py-2 px-4 bg-purple-500 rounded-lg text-white mb-3 hover:bg-purple-600 flex items-center space-x-2"
                    >
                      <StarIcon /> <span>Add Review & Rating</span>
                    </button>
                  </div>
